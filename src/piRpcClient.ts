...
export class PiRpcClient extends EventEmitter {
  ...
  constructor(piPath: string, extraArgs: string[], argsPrefix: string[] = []) {
    super();
    this.piPath = piPath;
  ...
  start(sessionPath?: string): void {
    ...
    const args = [...this.argsPrefix, '--mode', 'rpc'];
    if (sessionPath) {
    ...
    this.proc = spawn(this.piPath, args, {
      ...
      // Security: pass essential env vars
      env: {
    ...
    // stderr: log for debugging and capture for exit reporting
    this.proc.stderr!.on('data', (chunk: Buffer) => {
      const text = chunk.toString().trim();
      if (text) {
        console.warn('[pi stderr]', text);
    ...
    this.proc.on('exit', (code, signal) => {
      ...
      this.proc = null;
      this.emit('exit', code, signal, this.lastStderr);
      // Reject pending requests
      for (const [id, { reject }] of this.pendingRequests) {
        reject(new Error(`pi process exited (code=${code}): ${this.lastStderr || 'Unknown error'}`));
      }
